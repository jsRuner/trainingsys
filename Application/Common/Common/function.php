...
/**
     ...
     * @return string       用户昵称
     */
  function get_userphoto($uid = 0){
        static $list;
        if(!($uid && is_numeric($uid))){ //获取当前登录用户名
            return session('user_auth.middle_photo');
        }

        /* 获取缓存数据 */
        if(empty($list)){
            $list = S('sys_user_middle_photo_list');
        }
        ...
        if(isset($list[$key])){ //已缓存，直接使用
            ...
            $info = M('Member')->field('middle_photo')->find($uid);
            if($info !== false && $info['middle_photo'] ){
                $middle_photo = $info['middle_photo'];
                $name = $list[$key] = $middle_photo;
                /* 缓存用户 */
                $count = count($list);
                $max   = C('USER_MAX_CACHE');
                while ($count-- > $max) {
                ...
                S('sys_user_middle_photo_list', $list);
            } else {
                $name = '';
            }
        }
...
/**
 * 获得数据状态字符串
 * @param int 数据状态标识 分为 -1 0 1 
 * @return String 返回状态字符串
 * 
 */

...
use Student\Model\AuthRuleModel;
use Student\Model\AuthGroupModel;
...
//            redirect('Home/User/login');
            $this->error('你还没登陆',u('Home/User/login'));
        }
        /* 读取数据库中的配置 */
        ...
        $urlArray = array(
            ...
            'useremail' =>$user[2],
            'ts'=>'wangong',
            'sign' =>md5($user[0].$user[1].$user[2].'exam@phpems.net'.'wangong')
            );
        $parstr = http_build_query($urlArray);

            $this->assign('parstr',$parstr);

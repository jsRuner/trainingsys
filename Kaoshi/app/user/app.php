...
class app
{
	...
	public function login()
	{
		...
		//这里跳转
		Header("Location: /index.php?s=/Home/User/login.html");
		die();
		//获得请求的数组
		if($this->ev->get('userlogin'))
		{
			...
			if($user['userid'])
			{
				if($user['userpassword'] == md5($args['userpassword']))
				{
					...
					//这里是关键  todo  课堂系统也需要提供类似 的数据。
					$this->session->setSessionUser(array('sessionuserid'=>$user['userid'],
	...
	public function logout()
	{
		...
		//这里跳转:退出考试系统
		Header("Location: /index.php?s=/Student/MyClass/index.html");
		exit;
	}
}

			...
			<div class="col-md-9">
				<div class="panel panel-default">
					...
					<div class="panel-body">
						...
						<!-- 表单开始 -->
						<form role="form" id="myform" action="<?php echo u('Teacher/Course/editor');?>" method="post">
							<div class="form-group">
								<label for="course-title">标题</label>
								<input type="text" value="<?php echo ($courseData['title']); ?>" class="form-control" id="course-title" name="title" placeholder="输入课程标题" datatype="s5-16" errormsg="标题至少5个字符,最多16个字符!" nullmsg="你没有输入标题！" >
								<p class="help-block">课程的标题可以设置5到16个字符.</p>
							</div>
							<div class="form-group">
								<label for="course-about">介绍</label>
								<textarea name="about"  class="form-control" id="course-about" cols="30" rows="10" placeholder="输入课程介绍"><?php echo ($courseData['about']); ?></textarea>
								<p class="help-block">课程的介绍最多1000个汉字.</p>
							</div>
							<div class="form-group">
								<label for="course-price">价格</label>
								<input type="text" value="<?php echo ($courseData['price']); ?>" class="form-control" id="course-price" name="price" placeholder="0.00">
								...
								<div class="row">
									<div class="col-xs-4">
										<select name="category_id" id="course-category" class="form-control col-sm-2" placeholder="请选择">
											<?php if(is_array($categoryList)): $i = 0; $__LIST__ = $categoryList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['id']); ?>" <?php if($vo['id'] == $courseData['category_id']): ?>selected<?php endif; ?>   ><?php echo ($vo['title']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
										</select>
									</div>
									<!-- to:do 分类 -->
									<!-- <div class="col-xs-4">
